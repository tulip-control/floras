...
class MILP():
    ...
    def setup_model(self):
        ...
        Declares variables and bounds, adds constraints.
        """
        model_edges = self.model_edges
        model_nodes = self.model_nodes
        model_edges_without_I = list(self.G_minus_I.edges)
        model_nodes_without_I = list(self.G_minus_I.nodes)

        src = self.GD.init
        sink = self.GD.sink
        inter = self.cleaned_intermed

        s_sink = self.SD.acc_sys
        s_src = self.SD.init[0]

        model_s_edges = list(self.S.edges)
        model_s_nodes = list(self.S.nodes)

        self.model = Model()
        # Define variables
        f = self.model.addVars(model_edges, name="flow")
        m = self.model.addVars(model_nodes_without_I, name="m")
        d = self.model.addVars(model_edges, vtype=GRB.BINARY, name="d")
        ...
        self.model.addConstrs((d[i, j] >= 0 for (i,j) in model_edges), name='d_nonneg')
        self.model.addConstrs((m[i] >= 0 for i in model_nodes_without_I), name='mu_nonneg')
        self.model.addConstrs((f[i, j] >= 0 for (i,j) in model_edges), name='f_nonneg')

        # upper bounds
        self.model.addConstrs((d[i, j] <= 1 for (i,j) in model_edges), name='d_upper_b')
        self.model.addConstrs((m[i] <= 1 for i in model_nodes_without_I), name='mu_upper_b')
        # capacity (upper bound for f)
        self.model.addConstrs((f[i, j] <= 1 for (i,j) in model_edges), name='capacity')

        # preserve flow of at least 1
        self.model.addConstr((1 <= sum(f[i,j] for (i, j) in model_edges if i in src)), name='conserve_F')

        # conservation
        self.model.addConstrs((sum(f[i,j] for (i,j) in model_edges if j == l) == sum(f[i,j] for (i,j) in model_edges if i == l) for l in model_nodes if l not in src and l not in sink), name='conservation')

        # no flow into source or out of sink
        self.model.addConstrs((f[i,j] == 0 for (i,j) in model_edges if j in src or i in sink), name="no_out_sink_in_src")

        # cut constraint (cut edges have zero flow)
        self.model.addConstrs((f[i,j] + d[i,j] <= 1 for (i,j) in model_edges), name='cut_cons')
        ...
        self.model.addConstrs((d[i,j] - m[i] + m[j] >= 0 for (i,j) in model_edges_without_I))
